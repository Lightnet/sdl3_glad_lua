...
local window, err = sdl.init_window("sdl3 font 02", 800, 600, sdl.WINDOW_OPENGL + sdl.WINDOW_RESIZABLE)
if not window then
...
while running do
    ...
    for i = 1, #text do
        ...
        if x0 then
            -- Flip texture coordinates in Y (t0, t1) to correct upside-down text
            local t0_flipped = t1
            local t1_flipped = t0
            -- Triangle 1
            ...
            vertices[#vertices + 1] = t1_flipped
            x = x + x_advance
            lua_util.log(string.format("Char %d: x0=%.2f, y0=%.2f, x1=%.2f, y1=%.2f, s0=%.2f, t0=%.2f, s1=%.2f, t1=%.2f", char, x0, y0, x1, y1, s0, t0, s1, t1))
        else
